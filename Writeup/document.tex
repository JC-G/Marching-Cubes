...
\subsubsection{Implementation of Terrain Modification}
...
\end{lstlisting}
When a shape is added to the world, a new \texttt{Brush} object is created, and appended to a separate list. During the final iteration over the octree, as described in section \ref{section:octree_refinement}, the bounding box of the chunk at each leaf node of the octree is tested against the bounding box of each new \texttt{Brush} object. If these boxes intersect, then the geometry inside that chunk is regenerated. Listing \ref{edit_deleteregenphase} shows the code responsible for this.
