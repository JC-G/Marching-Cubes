...
\end{lstlisting}
When a shape is added to the world, a new \texttt{Brush} object is created, and appended the main list, as well as a separate list of new brushes. During the final iteration over the octree, as described in section \ref{section:octree_refinement}, the bounding box of the chunk at each leaf node of the octree is tested against the bounding box of each new \texttt{Brush} object. If these boxes intersect, then the geometry inside that chunk is regenerated. Listing \ref{edit_deleteregenphase} shows the code responsible for this.
...
%TODO - IQ reference
User interaction with the terrain modification system uses a set of pre-defined actions, defined through classes derived from a base \texttt{Action} class. The methods defined in such a class are shown in listing \ref{action_methods}

\begin{lstlisting}[language=C++,label={edit_deleteregenphase},caption={The methods of the \texttt{Action} class responsible for handling user interaction}]
...
\subsubsection{Limits of terrain modification}
 ...
 %TODO - GUI
 %TODO - where to put raycasting? relies on mapGeometry, currently explained in physics
