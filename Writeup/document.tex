...
bool Octree::flagSplitPhase(glm::vec3 inPos) {
  ...
  \begin{lstlisting}[language=C++,label={refinephase},caption={The second stage in the octree refinement process. \texttt{edgeNeighbors} gives the relative positions of the 6 neighboring nodes at the same level, and \texttt{childPosition} gives the position of the child to check in each neighbor. The flow of this code is designed such that exactly the 4 children of the neighboring node that touch the chunk in this node are checked.}] 
