...
With an octree, any condition could be used to determine the level of detail at any given point. We will use this in section \ref{section:physics_meshes} to control the level of detail around physics objects, for consistent collision detection. We will control the level of detail using the function \texttt{shouldSplit}, which returns whether a leaf node should split into its 8 children, and \texttt{shouldChop}, which returns whether a non-leaf node should become a leaf. When the octree changes, geometry associated with octree leaves that became branches, or were removed from the octree, is deleted, and new geometry is generated at each new leaf.
...
\begin{figure}[H]
  ...
  \caption{An example demonstrating the cracks between levels of detail. Here, parts of the blue background can be seen in the gaps between the chunks.}
  \label{fig:cracks2}
\end{figure}
...
\begin{SCfigure}[][!h]
  \caption{Cell vertex naming convention for transition cells in the Transvoxel algorithm. (a) shows the higher resolution face of the cell, and (b) shows the opposite, lower resolution face. Reproduced from figure 4.16 of the Transvoxel paper\cite{lengyel_2010}}
...
We now describe a Transvoxel implementation on the GPU, using GLSL, making use of the modified lookup tables from section \ref{section:lookup_tables}. The algorithm is implemented in 3 separate compute shader phases, executed in order, to generate the geometry for a single chunk.
...
\item \underline{Counting phase}: For each regular cell, calculate the number of mesh triangles within it, and if this number is non-zero (the cell is not all air, or all solid), add it to the list of cells to generate geometry for in the next phase. Listing \ref{tv_count_regular} shows the code responsible for this.
